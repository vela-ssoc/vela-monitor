...
// 网络基础指标定义  (全局变量)
var (
	netBytesSent   = metrics.NewSimpleGauge("net_bytes_sent_total", "发送的总字节数", getNetBytesSent)
	netBytesRecv   = metrics.NewSimpleGauge("net_bytes_recv_total", "接收的总字节数", getNetBytesRecv)
	netPacketsSent = metrics.NewSimpleGauge("net_packets_sent_total", "发送的总数据包数", getNetPacketsSent)
	netPacketsRecv = metrics.NewSimpleGauge("net_packets_recv_total", "接收的总数据包数", getNetPacketsRecv)
	netErrin       = metrics.NewSimpleGauge("net_err_in_total", "接收错误总数", getNetErrin)
	netErrout      = metrics.NewSimpleGauge("net_err_out_total", "发送错误总数", getNetErrout)
	netDropin      = metrics.NewSimpleGauge("net_drop_in_total", "接收丢弃的数据包总数", getNetDropin)
	netDropout     = metrics.NewSimpleGauge("net_drop_out_total", "发送丢弃的数据包总数", getNetDropout)
	netFifoin      = metrics.NewSimpleGauge("net_fifo_in_total", "FIFO缓冲区接收错误数", getNetFifoin)
	netFifoout     = metrics.NewSimpleGauge("net_fifo_out_total", "FIFO缓冲区发送错误数", getNetFifoout)
)
